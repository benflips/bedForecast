...
#splits symptomatic into critical, severe and mild
  #accounts for detection, and 
  # moves them forward tth = time to hospitalisation
splitter <- function(active, time, tth = 6, tInICU = 8, fracSev = 0.15, fracCrit = 0.05, det = 0.36){
  newCases <- c(0, diff(active))
  nn <- length(newCases)
  newCases[newCases<0]<-0 # catch negatives
  newCases <- newCases/det # account for detection
  hDate <- time + tth # expected hositalisation date
  icuDate <- time + tth + 6 #additional 6 days in hospital before ICU
  sev <- (fracSev+fracCrit)*newCases # severe plus critical, hospital burden
  sev <- cumsum(sev)
  sev <- sev-c(rep(0, 6), sev[-((nn-5):nn)]) #discharge or ICU after 6 days
  crit <- fracCrit*newCases
  crit <- cumsum(crit)
  crit <-crit - c(rep(0, tInICU), crit[-((nn-(tInICU-1)):nn)])
  
  list(hDate = hDate, icuDate = icuDate, severe = sev, critical = crit)
}
